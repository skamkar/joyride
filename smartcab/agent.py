...
class LearningAgent(Agent):
    ...
    def __init__(self, env, learning=False, epsilon=1.0, alpha=0.5, epsilon_step=0.0025, alpha_step=0.0025):
    ...
    def build_state(self):
        ...
        # Oncoming left turns are not necessary for safety
    ...
    def actions_of_max_Q(self, state, maxQ):
        """ Given a state and maxQ value, it returns all actions that have Q values
            equal to maxQ. To avoid tolerance issues when comparing two float values,
            the is_close function is employed """
        actions = []
        for action in self.Q[state]:
            if self.is_close(self.Q[state][action], maxQ):
                actions.append(action)
        return actions

    def is_close(self, a, b, rel_tol=1e-04, abs_tol=1e-04):
        """ Given an absolute and relative tolerance, confirms two floats are equivalent """
    ...
    def choose_action(self, state):
        ...
        if self.learning:
            if random.random() < self.epsilon:
                ...
                action = random.choice(self.actions_of_max_Q(state, self.get_maxQ(state)))
        else:
            action = random.choice(self.valid_actions)
